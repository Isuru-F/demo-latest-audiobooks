...
const filteredAudiobooks = computed(() => {
  ...
  if (!query && !multiCastOnly.value) {
    return spotifyStore.audiobooks;
  }
  
  return spotifyStore.audiobooks.filter(audiobook => {
    // Apply multi-cast filter first
    if (multiCastOnly.value && !isMultiCast(audiobook)) {
      return false;
    }
    ...
    // Search by author name with null safety
    const authorMatch = (audiobook.authors ?? []).some(author => 
      author.name?.toLowerCase().includes(query)
    );
    
    // Search by narrator with null safety
          ...
          <div class="toggle-container">
            <label class="toggle-label">
              <input 
                ...
                aria-label="Show only audiobooks with multiple narrators"
              />
