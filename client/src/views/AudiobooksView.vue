...
const multiCastCount = computed(() => {
  return spotifyStore.audiobooks.filter(audiobook => 
    audiobook.narrators && audiobook.narrators.length > 1
  ).length;
});
    ...
    <section class="audiobooks">
        ...
        <div class="search-filter-container">
          ...
          <div class="multi-cast-toggle">
            <label class="toggle-label">
              ...
              <span v-if="multiCastCount > 0" class="count-badge">({{ multiCastCount }})</span>
            </label>
          </div>
        </div>
      </div>
      ...
      <div v-else>
        <p v-if="filteredAudiobooks.length === 0" class="no-results">
          <span v-if="multiCastOnly && !searchQuery.trim()">
            No multi-cast audiobooks found.
          </span>
          <span v-else-if="multiCastOnly && searchQuery.trim()">
